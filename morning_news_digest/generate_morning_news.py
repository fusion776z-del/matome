...
def make_simple_summary(title: str, description: str, category: str) -> tuple[str, str]:
    ...
    why = why_map.get(category, "関心キーワードや生活・仕事への関連度を確認する価値があります。")
    return summary, why
...
def parse_feed(xml_bytes: bytes, feed: dict, config: dict) -> list[dict]:
    ...
    if not items:
        items = [elem for elem in root.iter() if local_name(elem.tag) == "entry"]
...
def call_ai_aggregate_once(client, model: str, group: dict, config: dict, retry: bool = False) -> dict:
    prompt = build_aggregate_prompt(group, config=config, retry=retry)
...
def render_topic_card(group: dict, config: dict) -> str:
    ...
    related = render_related_links(articles, config)
...
def css() -> str:
    ...
    .news { border-left:4px solid var(--accent); padding:14px 16px; background:#fffdf7; border-radius:12px; border:1px solid var(--line); margin:12px 0; }
    .title { font-weight:800; font-size:18px; margin:4px 0 10px; }
    .source { font-size:12px; color:var(--muted); margin-top:8px; }
    .score { font-weight:800; color:var(--green); }
    .score.mid { color:var(--yellow); }
    .score.low { color:var(--red); }
    .topic-meta { display:flex; gap:8px; flex-wrap:wrap; align-items:center; font-size:12px; color:var(--muted); }
    .badge { padding:2px 8px; border-radius:999px; background:#eef3ff; color:#315dcc; border:1px solid #d9e4ff; font-weight:700; }
    .confidence { padding:2px 8px; border-radius:999px; border:1px solid var(--line); font-weight:700; }
    .confidence.high { background:#edf8f1; color:#2f8f5b; }
    .confidence.medium { background:#fff8e5; color:#9a7300; }
    .confidence.low { background:#fff0f0; color:#c64545; }
    .source-inline { color:var(--muted); }
    .related { margin:10px 0 0 0; padding-left:20px; font-size:13px; }
    .related a { color:#315dcc; text-decoration:none; }

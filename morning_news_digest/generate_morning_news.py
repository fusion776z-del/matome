...
RSSを取得し、AI要約付きのHTMLを生成します。
- OPENAI_API_KEY が設定されている場合: AI要約を使う
- OPENAI_API_KEY が未設定/失敗した場合: 簡易要約にフォールバック
"""
...
def ai_summary(title: str, description: str, category: str, config: dict) -> tuple[str, str]:
    """AIで「何が起きたか」「なぜ重要か」を作る。失敗時は簡易要約に戻す。"""
...
def enrich_articles_with_ai(articles: list[dict], config: dict) -> list[dict]:
    ...
    for index, article in enumerate(target_articles, start=1):
        summary, why = ai_summary(article["title"], article.get("description", ""), article["category"], config)
...
def save_articles(articles: list[dict]) -> None:
    with sqlite3.connect(DB_PATH) as conn:
        ...
        conn.executemany(
            ...
            [
                (
                    a["url"], a["title"], a["source"], a["category"], a["published_at"],
                    a["description"], a["summary"], a["why"], a["score"], now,
                )
                for a in articles
            ],
        )
...
def render_article_card(article: dict) -> str:
    return f"""
    <article class="news">
      <div class="title"><a href="{escape(article['url'])}" target="_blank" rel="noopener noreferrer">{escape(article['title'])}</a></div>
      <p><strong>何が起きたか:</strong> {escape(article['summary'])}</p>
      <p><strong>なぜ重要か:</strong> {escape(article['why'])}</p>
      <div class="source">
        出典: {escape(article['source'])} / カテゴリ: {escape(article['category'])} /
...
def css() -> str:
    ...
    body{margin:0;background:var(--bg);color:var(--ink);font-family:-apple-system,BlinkMacSystemFont,"Segoe UI","Hiragino Sans","Yu Gothic UI",Meiryo,sans-serif;line-height:1.7}
    .app{display:grid;grid-template-columns:280px 1fr;min-height:100vh}
    aside{padding:28px 20px;border-right:1px solid var(--line);background:var(--paper);position:sticky;top:0;height:100vh;box-sizing:border-box}
    main{padding:40px clamp(22px,5vw,72px)}
    .page{max-width:1040px;margin:0 auto}
    .hero,.card{background:var(--paper);border:1px solid var(--line);border-radius:22px;padding:24px;box-shadow:0 10px 30px rgba(39,33,25,.08);margin-bottom:16px}
    .nav-item{display:block;padding:8px 10px;color:#38332e;text-decoration:none;border-radius:10px}
    .nav-item:hover{background:#f0ede6}
    .pill{display:inline-block;margin:4px;padding:6px 10px;border-radius:999px;background:#f3f0e9;border:1px solid var(--line);font-size:13px}
    .news{border-left:4px solid var(--accent);padding:12px 14px;background:#fffdf7;border-radius:12px;border:1px solid var(--line);margin:10px 0}
    .title{font-weight:800;font-size:18px}.title a{color:var(--ink);text-decoration:none}.title a:hover{text-decoration:underline}
    .source{font-size:12px;color:var(--muted);margin-top:8px}
    .score{font-weight:800;color:var(--green)}.score.mid{color:var(--yellow)}.score.low{color:var(--red)}
    @media(max-width:900px){.app{grid-template-columns:1fr}aside{position:static;height:auto;border-right:none;border-bottom:1px solid var(--line)}}
    """
...
def render_html(articles: list[dict], config: dict) -> str:
    ...
    preferred_categories = ["国内", "AI・テック", "ビジネス", "北海道・函館", "未分類"]
    all_categories = preferred_categories + sorted({a["category"] for a in articles if a["category"] not in set(preferred_categories)})

...
def render_html(articles, config):
    ...
    return f'''<!doctype html><html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>{title}</title><style>{css()}</style></head><body><div class="app"><aside><h2>🗞️ Morning Digest</h2><a class="nav-item" href="#overview">🏠 概要</a><a class="nav-item" href="#digest">☕ 今日の朝刊</a></aside><main><section class="page" id="overview"><div class="hero"><div style="font-size:46px">☕</div><h1>{title}</h1><p>RSSから記事を取得し、Notion/OneNote風HTMLとして出力しました。</p><span class="pill">📅 生成日時: {escape(now_label)}</span><span class="pill">📰 表示記事: {len(articles)}件</span></div><h2 id="digest">今日押さえるべき{top_pick_count}つ</h2><div class="card">{top_cards}</div><h2>カテゴリ別ニュース</h2>{''.join(sections)}</section></main></div></body></html>'''
